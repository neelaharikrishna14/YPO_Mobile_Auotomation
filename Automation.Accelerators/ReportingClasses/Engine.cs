...
namespace Selenium.Automation.Accelerators
{
	public class Engine
	{
		...
		public Engine(String resultPath, String serverName)
		{
            ...
            this.timestamp = TimeZoneInfo.ConvertTime(DateTime.Now, TimeZoneInfo.Local, TimeZoneInfo.Local).ToString("MM-dd-yyyy HH-mm-ss");
		...
		public void PublishIteration(Iteration iteration)
		{
            ...
            <body>
	            <div class='container'>
		            <div style='padding-top: 5px; padding-bottom:5px;'>
			            <img src='https://www.Selenium.com/images/qc_logo_02.gif' style='padding-top:20px; width:auto; height:auto;' />
			...
			foreach (Chapter chapter in iteration.Chapters)
			{
				...
				{
					builder.AppendFormat("<div class='wrapper'><p class='Report-Step'>Step {2}: {0}<span class='pull-right'><span class='glyphicon glyphicon-{1}'></span></span></p>", step.Title, step.IsSuccess ? "ok green" : "remove red",stepInc);
					...
					foreach (Act action in step.Actions)
					{
						...
						else
						{
							...
							builder.Append("<a href='" + action.ImagePath + "'><span class='glyphicon glyphicon-paperclip normal'></span></a>&nbsp;");
                            //builder.Append("<a href='" + Path.Combine("Screenshots", String.Format("{0} {1} {2} Error.png", iteration.Browser.TestCase.Title, iteration.Browser.Title, iteration.Title)) + "'><span class='glyphicon glyphicon-paperclip normal'></span></a>&nbsp;");
			...
			template = template.Replace("{{EXECUTION_DURATION}}", iteration.EndTime.Subtract(iteration.StartTime).ToString(@"hh\:mm\:ss"));
		...
		public void Summarize(bool isFinal = true)
		{
			...
			foreach (TestCase testCase in Reporter.TestCases)
			{
				foreach (Browser browser in testCase.Browsers)
				{
					...
					{                       
                        ...
                        builder.AppendFormat("<td> <table> <tr> <td class='wrapword'> {0} </td>  </tr> </table> </td>", testCase.Name);
                        builder.AppendFormat("<td> <table> <tr> <td> {0} </td>  </tr> </table> </td>", testCase.RequirementFeature);
                        builder.AppendFormat("<td> <table> <tr> <td> {0} - {1} </td>  </tr> </table> </td>", browser.BrowserName, browser.BrowserVersion);
                        ...
                        builder.AppendFormat("<td> <table> <tr> <td> {0} </td>  </tr> </table> </td>", iteration.BugInfo);
                        ...
                        //builder.AppendFormat("<td><a href='{0}' target='_blank'>{1}</a></td>", strConfluenceURL, testCase.Name);
                        //builder.AppendFormat("<td>{0}</td>", testCase.RequirementFeature);
                        ...
                        //builder.AppendFormat("<td><a href='{0}' target='_blank'><span class='glyphicon glyphicon-{1}'></span></a></td>", String.Format("{0} {1} {2}.html", testCase.Title, browser.Title, iteration.Title), iteration.IsSuccess == true ? "ok green" : "remove red");
			...
			template = template.Replace("{{EXECUTION_DURATION}}", LastCaseEndTime.Subtract(FirstCaseBeginTime).ToString(@"hh\:mm\:ss"));
			template = template.Replace("{{EXECUTION_DURATION_CUM}}", ExecutionTimeCumulative.ToString(@"hh\:mm\:ss"));
